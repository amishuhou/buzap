...
namespace UsedParts.UI.Interactions.Behaviors
{
    ...
    public abstract class SafeBehavior<T> : Behavior<T> where T : FrameworkElement
    {
        ...
        private void AssociatedObjectLoaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                ParentPage = (PhoneApplicationFrame)Application.Current.RootVisual;
                _pageSource = ParentPage.CurrentSource;

                var weakListener = new WeakEventListener<SafeBehavior<T>, PhoneApplicationFrame, NavigationEventArgs>(this, ParentPage)
                {
                    OnEventAction = (behavior, o, arg3) => behavior.ParentPageNavigated(o, arg3)
                };
                ParentPage.Navigated += weakListener.OnEvent;
            }
            OnSetup();
        }
